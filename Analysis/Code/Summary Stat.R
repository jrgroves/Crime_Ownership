...
#Data: Census Sections####

    temp.a <- core.tract %>%
      select(year, GEOID, Tot_Pop, Med_Inc, dens, starts_with("per")) %>%
      distinct(GEOID, year, .keep_all = TRUE) %>%
      filter(year > 2017) %>%
      filter(year < 2025) %>%
      pivot_longer(cols = -c(GEOID, year), names_to = "type", values_to = "rate")
...
#Grid level aggregation  
  temp.a <- core.grid %>%
    ...
    filter(year > 2017) %>%
    filter(year < 2025) %>%
    pivot_longer(cols = -c(grid_id, year), names_to = "type", values_to = "rate")
...
flextable(TEMP) %>%
  separate_header() %>%
  add_header_lines(values = "Table 4: Census Summary by Census Tract and Grid") %>%
  align(align = "center", part = "header") %>%
  colformat_double(digits = 4) %>%
  merge_at(i = 1:7, j = "type") %>%
  merge_at(i = 8:14, j = "type") %>%
